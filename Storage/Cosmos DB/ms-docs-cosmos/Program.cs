...
//Create a properties object for a new container using the ContainerProperties type. Store the result in a variable named properties. Be sure to set these parameters:
ContainerProperties properties = new(
    id: "products",
    partitionKeyPath: "/categoryId"
);
//Create an autoscale throughput object using the CreateAutoscaleThroughput(Int32) static method. Store the result in a variable named throughput. Be sure to set these parameters:
var throughput = ThroughputProperties.CreateAutoscaleThroughput(
    autoscaleMaxThroughput: 1000
);

//Create, or get, a new container by calling CreateContainerIfNotExistsAsync(String, String, Nullable<Int32>, RequestOptions, CancellationToken). Store the result in a variable named container. Be sure to set these parameters:
Container container = await database.CreateContainerIfNotExistsAsync(
    containerProperties: properties,
    throughputProperties: throughput
);
//Now, output the unique identifier for the container.
